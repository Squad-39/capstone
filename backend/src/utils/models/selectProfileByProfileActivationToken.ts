...
export async function selectProfileByProfileActivationToken(profileActivationToken: string): Promise<Profile|null> {
	try {
		...
		const mysqlQuery: string = "SELECT BIN_TO_UUID(profileId) as profileId,  profileAtHandle, profileAvatarUrl, profileEmail, profilePhone FROM models WHERE profileActivationToken = :profileActivationToken"
