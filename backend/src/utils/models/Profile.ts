...
export async function selectPartialProfilesBySquadId (squadId: string): Promise<PartialProfile[]> {
   return sql <PartialProfile[]>`SELECT "profileId", "profileEmail","profileName" FROM profile INNER JOIN "request" ON   profile."profileId" = request."requestProfileId"  WHERE "request.requestProfileSquadId"."requestSquadId" = ${squadId}`
}
